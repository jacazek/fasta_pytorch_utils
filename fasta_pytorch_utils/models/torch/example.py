...
class ExampleSequenceModel(nn.Module):
    def __init__(self, vocabulary, embedding_dimension, device="cpu", model_type=ModelType.LSTM):
        super(ExampleSequenceModel, self).__init__()
        self.device = device
        ...
        self.rnn = rnn_layer(embedding_dimension, self.hidden_size, batch_first=True,
                             device=self.device)
        self.linear = torch.nn.Linear(self.hidden_size, 1, device=self.device)
    ...
    def forward(self, input_tensor, lengths):
        ...
        final_hidden_state = unpacked[torch.arange(unpacked.size(0)), lengths - 1]
...
class SplitLSTMModel(nn.Module):
    def __init__(self, vocabulary, embedding_dimension, device="cpu", model_type=ModelType.LSTM):
        ...
        self.linearA = torch.nn.Linear(self.hidden_size, self.hidden_size, device=self.device)
        self.linearB = torch.nn.Linear(self.hidden_size, self.hidden_size , device=self.device)
        self.linear = torch.nn.Linear(self.hidden_size * 2, 1, device=self.device)
        # self.activation = torch.nn.Sigmoid()
...
class ConvoludedLSTMModel(nn.Module):
    def __init__(self, vocabulary, embedding_dimension, device="cpu", model_type=ModelType.LSTM):
        ...
        self.rnn = rnn_layer(embedding_dimension, self.hidden_size, batch_first=True,
                             device=self.device)
        self.conv1d_1 = torch.nn.Conv1d(self.hidden_size, self.hidden_size, kernel_size=9, device=self.device)
        self.layer_normalization = torch.nn.LayerNorm(self.hidden_size, device=self.device)
        self.linearA = torch.nn.Linear(self.hidden_size, self.hidden_size * 2, device=self.device)
        self.relu = torch.nn.ReLU()
        self.linear = torch.nn.Linear(self.hidden_size*4, 1, device=self.device)
        # self.activation = torch.nn.Sigmoid()
